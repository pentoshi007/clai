...
try {
  ...
  await settle(() => setup.mockInput.pressKey("i"));
  await settle(() => setup.mockInput.pressKey("\x1b[D"));
  assert.ok(setup.captureCharFrame().includes("h▎i"), "left arrow must move the caret into the text");
  await settle(() => setup.mockInput.pressKey("\x1b[3~"));
  assert.ok(setup.captureCharFrame().includes("h▎"), "delete must remove the character under the caret");
  await settle(() => setup.mockInput.pressKey("\x1b[H"));
  assert.ok(setup.captureCharFrame().includes("▎h"), "home must move the caret to the start");
  await settle(() => setup.mockInput.pressKey("x"));
  assert.ok(setup.captureCharFrame().includes("x▎h"), "typing must insert at the caret");
  await settle(() => setup.mockInput.pressKey("\x7f"));
  assert.ok(setup.captureCharFrame().includes("▎h"), "backspace must remove the character before the caret");
  await settle(() => setup.mockInput.pressKey("\x1b[F"));
  await settle(() => setup.mockInput.pressKey("s"));
  assert.ok(setup.captureCharFrame().includes("hs▎"), "end must move the caret to the tail");
  await settle(() => setup.mockInput.pressEscape());
