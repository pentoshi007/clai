...
export const mimoProvider: LlmProvider = {
  ...
  defaultModel: defaultModels.mimo,
  envVar: "MIMO_API_KEY",
  validateKey: (key: string) => /^(?:sk|tp)-[A-Za-z0-9_-]{8,}$/.test(key),
  async listModels(auth: ProviderAuth): Promise<string[]> {
    if (!auth.apiKey) throw new Error("Xiaomi MiMo API key is required");
    const now = Date.now();
    ...
    const models = ingestOpenAiModelCatalog("mimo", data);
    if (models.length > 0) {
      cachedModels = models;
      lastFetchTime = now;
    }
  ...
  async complete(
    ...
    if (!auth.apiKey) throw new Error("Xiaomi MiMo API key is required");
    const model = request.model ?? defaultModels.mimo;
    ...
    return toCompletionResult("mimo", model, payload);
  },
  async stream(
    request: CompletionRequest,
    auth: ProviderAuth,
