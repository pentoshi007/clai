...
import { destructiveCommandPatterns, exfiltrationPatterns, networkScanTools } from './patterns.js';
...
export function classifyToolCall(call: ToolCall): RiskDecision {
  if (call.name === 'sysinfo' || call.name === 'fs.read' || call.name === 'fs.list' || call.name === 'fs.search') {
    return { level: 'safe', reason: 'Read-only operation' };
  }

  if (call.name === 'http.fetch') {
    return { level: 'safe', reason: 'HTTP fetch is read-only with response size limits' };
  }

  if (call.name === 'shell.exec') {
    const command = stringArg(call.args, 'command') ?? '';
    if (destructiveCommandPatterns.some((pattern) => pattern.test(command))) {
      return { level: 'block', reason: 'Command matches destructive safety pattern' };
    }
    if (exfiltrationPatterns.some((pattern) => pattern.test(command))) {
      return { level: 'block', reason: 'Command resembles secret or data exfiltration' };
    }
    if (commandContainsNetworkScanner(command) && containsPublicTarget(command) && !command.includes('--i-own-this')) {
      return { level: 'block', reason: 'Public target scanning requires explicit --i-own-this authorization flag' };
    }
  ...
  if (call.name === 'net.scan') {
    ...
    if (target && !isPrivateIpv4(target) && !ownsTarget) {
      return { level: 'block', reason: 'Public IP scan requires ownership confirmation' };
    }
    return { level: 'confirm', reason: 'Network scans require confirmation' };
  }

  if (call.name === 'fs.write' || call.name === 'pkg.install') {
    return { level: 'confirm', reason: 'Mutating operation requires confirmation' };
  }
