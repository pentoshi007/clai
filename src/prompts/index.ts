...
- plan.create: {"goal":"<short goal>","detail":"<stack/approach chosen and why, architecture, how you'll verify>","tasks":["task 1","task 2", ...],"kind":"coding|pentest|general"} — **primary tool for multi-step work**. Use it for project builds (scaffold → implement → verify) AND for pentest engagements (after recon, lock in attack surface + tasks). Kind is coding for software builds, pentest for security work, general otherwise. The plan is saved durably, re-shown every turn, and SURVIVES context compaction. After creating it, STOP and wait — the user is asked to approve (implement) or discard it.
